         ...
         using( SQLiteConnection connection = new SQLiteConnection(cadenaConexion))
            {
                ...
                var usuarioAModificar = usuarios.FirstOrDefault(usuario => usuario.Id == usuario.Id);
                command.Parameters.Add(new SQLiteParameter("@nombreDeUsuario",usuarioAModificar.NombreDeUsuario));     
                connection.Open();
                command.ExecuteNonQuery();
                connection.Close();
            }
