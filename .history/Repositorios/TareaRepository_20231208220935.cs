...
{   
    ...
    public List<Tarea> GetAllTareasDeUnUsuario1(int idUsuario){
            ...
            command.CommandText = "SELECT * FROM Tablero WHERE id_usuario_asignado = @idUsuario;";
            command.Parameters.Add(new SQLiteParameter("@idtablero_prop",idUsuario));    // Lo que esta en @idUsu se reemplaza por lo que esta en id, se usa para evitar inyeccion de sql para no usar {}
            connection.Open();
            using(SQLiteDataReader reader = command.ExecuteReader())
            {
