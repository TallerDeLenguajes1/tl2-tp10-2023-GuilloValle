...
public class UsuarioRepository : IUsuarioRepository
{
    ...
    public void CrearNuevoUsuario(Usuario usuario){
            ...
            var query = $"INSERT INTO Usuario (Nombre_de_usuario,Password) VALUES (@Nombre_de_usuario,@Password)";
            using (SQLiteConnection connection = new SQLiteConnection(cadenaConexion))
            {
    ...
    public void getUsuarioLogin(string nombre, string Password){

        SQLiteConnection connection = new SQLiteConnection(cadenaConexion);  //Me conecto con la base de datos (en este caso Kanban.db)
        var usuario = new Usuario();
        SQLiteCommand command = connection.CreateCommand();  //creo un comando para poder hacer una consulta SQL
        command.CommandText = "SELECT Nombre_de_usuario, password FROM Usuario WHERE Nombre_de_usuario = @nombre AND password = @pass";
